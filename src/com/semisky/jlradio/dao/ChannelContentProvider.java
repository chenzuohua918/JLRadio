...
public class ChannelContentProvider extends ContentProvider {
	...
	public static final int COLLECT_LIST_CODE = 1;
	public static final int COLLECT_ITEM_CODE = 2;
	public static final int FM_LIST_CODE = 3;
	public static final int FM_ITEM_CODE = 4;
	public static final int AM_LIST_CODE = 5;
	public static final int AM_ITEM_CODE = 6;
	private SQLiteOpenHelper helper;
	private static Map<String, String> collectColumnMap = new HashMap<String, String>();
	...
	static {
		...
		uriMatcher.addURI(DBConfiguration.AUTHORITY, "collect_channels",
				COLLECT_LIST_CODE);// 添加需要匹配uri，如果匹配就会返回匹配码
		uriMatcher.addURI(DBConfiguration.AUTHORITY, "collect_channels/#",
				COLLECT_ITEM_CODE);// #号为通配符
		...
		collectColumnMap.put(DBConfiguration.TableCollectConfiguration._ID,
				DBConfiguration.TableCollectConfiguration._ID);
		collectColumnMap.put(
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case COLLECT_LIST_CODE:// 查整个
			// 设置查询的表
			builder.setTables(DBConfiguration.TableCollectConfiguration.TABLE_NAME);
			// 设置投影映射
			builder.setProjectionMap(collectColumnMap);

			if (TextUtils.isEmpty(sortOrder)) {
				orderBy = DBConfiguration.TableCollectConfiguration.DEFAULT_SORT_ORDER;
			} else {
				orderBy = sortOrder;
			}

			cursor = builder.query(database, null, null, null, null, null,
					orderBy);
			break;
		case COLLECT_ITEM_CODE:// 根据条件查询
			// 设置查询的表
			builder.setTables(DBConfiguration.TableCollectConfiguration.TABLE_NAME);
			// 设置投影映射
			builder.setProjectionMap(collectColumnMap);
	...
	public String getType(Uri uri) {
		switch (uriMatcher.match(uri)) {
		case COLLECT_LIST_CODE:
			return DBConfiguration.TableCollectConfiguration.CONTENT_TYPE_DIR;
		case COLLECT_ITEM_CODE:
			return DBConfiguration.TableCollectConfiguration.CONTENT_TYPE_ITEM;
		case FM_LIST_CODE:
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case COLLECT_LIST_CODE:
			...
			rowId = database.insert(
					DBConfiguration.TableCollectConfiguration.TABLE_NAME, null,
					values);
			if (rowId > 0) {
